...
public interface Reducer<T, V> {
    ...
    static <T> Transducer<T, ?, String> join(String seq, Function<T, String> function) {
        return Transducer.of(() -> new StringJoiner(seq), (j, t) -> j.add(function.apply(t)), StringJoiner::toString);
    }
    ...
    static <T, E, V> Reducer<T, V> mapping(Function<T, E> mapper, Reducer<E, V> reducer) {
        ...
        return of(reducer.supplier(), (v, t) -> {
            accumulator.accept(v, mapper.apply(t));
        }, reducer.finisher());
    }
